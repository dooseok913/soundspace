...
- **멀티모델 앙상블**: 오디오 특성(KNN) + 텍스트 임베딩(Sentence-BERT) 결합
- **393차원 하이브리드 벡터**: 시맨틱 의미와 오디오 특성 통합
- **RAG 기반 설명 생성**: 개인화된 추천 이유 제공
...
| 모델 | NDCG@50 | 향상률 |
|------|---------|--------|
| Baseline (Manhattan) | 0.011 | - |
| KNN 단일 모달 | 0.498 | 45배 |
| 앙상블 (KNN + SBERT) | 0.571 | **50배** |
...
### 3. Spotify API 제약 대응
- 오디오 특성 API 중단 → ISRC 기반 외부 데이터 매핑
- 캐글 과거 데이터(11만 건)로 역추정 모델 학습
